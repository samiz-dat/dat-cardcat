...
class DatWrapper {
  ...
  create() {const createDatAsync = _bluebird2.default.promisify(_datNode2.default);return createDatAsync(this.directory, this.opts);}importFiles() {return new _bluebird2.default((resolve, reject) => {const dat = this.dat;if (this.dat.owner) {const importer = dat.importFiles(this.directory, () => {console.log(`Finished importing files in ${this.directory}`);resolve(true);});importer.on('error', reject);} else {resolve(false);}});} // Lists the contents of a dat
