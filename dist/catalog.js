...
class Catalog extends _events2.default {constructor(baseDir) {var _this;_this = super();this.attachEventListenersAndJoinNetwork = dat => {dat.on('import', this.handleDatImportEvent);dat.on('download metadata', this.handleDatDownloadMetadataEvent);dat.on('sync metadata', this.handleDatSyncMetadataEvent); // dat.on('sync collections', this.handleDatSyncCollectionsEvent);
      return dat.run();};this.ingestDatFile = (() => {var _ref = _asyncToGenerator(function* (data, attempts = 10) {// console.log('trying to import:', data.type, ':', data.file, data.progress);
        const entry = (0, _importers2.default)(data.file, 'calibre');if (entry) {const downloaded = yield _this.multidat.getDat(data.key).hasFile(data.file);const downloadedStr = downloaded ? '[*]' : '[ ]'; // console.log(chalk.bold('adding:'), downloadedStr, data.file);
          const text = _extends({ dat: data.key, state: data.type === 'put' }, entry, { downloaded });return _this.db.addTextFromMetadata(text).then(function () {return _this.emit('import', _extends({}, text, { progress: data.progress }));}).then(function () {console.log(`${data.progress.toFixed(2)}%`, 'adding:', downloadedStr, data.file);}).catch(function (e) {if (attempts > 0) {console.log('retry', attempts);return _bluebird2.default.delay(1000).then(function () {return _this.ingestDatFile(data, attempts - 1);});}console.error('errrored', e);return null;});}return _bluebird2.default.resolve(false);});return function (_x) {return _ref.apply(this, arguments);};})();this.handleDatImportEvent = data => {console.log(`${data.progress.toFixed(2)}%`, 'import download event.', data.type, ':', data.file);const text = _extends({ dat: data.key, state: data.type === 'put' }, (0, _importers2.default)(data.file, 'calibre'), { downloaded: true // downloaed is true as you are importing it, right?
      ...
      this.db.addTextFromMetadata(text).then(() => this.emit('import', _extends({}, text, { progress: data.progress }))).catch(console.error);};this.handleDatDownloadMetadataEvent = data => {console.log(`${data.progress.toFixed(2)}%`, 'Metadata download event.', data.type, ':', data.file);const text = _extends({ dat: data.key, state: data.type === 'put' }, (0, _importers2.default)(data.file, 'calibre'), { downloaded: false // need to check for downloaded - probaby at this point does not makes sense as we have not even downloaded the metadata.
      }); // if this times out we should implement a simple promise queue,
      // so that we just these requests to a list that gets executed when
      // the preceeding functions .then is called.
      this.db.addTextFromMetadata(text).catch(console.error);};this.handleDatSyncMetadataEvent = dat => {console.log('Metadata sync event. Ingesting contents for:', dat); // this.ingestDatContents(dw);
    };this.handleDatListingEvent = data => {console.log('Importing: ', data);};this.handleDatListingEndEvent = data => {console.log(data);};this.handleDatSyncCollectionsEvent = dw => {console.log('Collections sync event. Ingesting collections for:', dw.name);this.ingestDatCollections(dw);};this.baseDir = baseDir;this.dats = [];this.db = new _db2.default(_path2.default.format({ dir: this.baseDir, base: 'catalog.db' }));this.multidat = new _multidat2.default(baseDir);this.isReady = false; // For bulk imports we'll use queue
    ...
    return this.multidat.close().then(() => this.emit('closed')).catch(err => this.emit('error', err));}initDatabase() {return this.db.init();}initMultidat() {return this.multidat.init().then(() => this.getDats()).then(dats => this.multidat.initOthers(dats)).then(() => this.cleanupDatRegistry()).then(() => this.multidat.getDats()).each(dw => this.registerDat(dw)).each(dw => this.attachEventListenersAndJoinNetwork(dw)); // .each(dw => this.ingestDatContents(dw));
