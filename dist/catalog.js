...
class Catalog {constructor(baseDir) {this.getDats = () => this.db('dats').select();this.getDat = key => this.db('dats').select().where('dat', key);this.baseDir = baseDir;this.dats = [];this.db = (0, _knex2.default)({ client: 'sqlite3', connection: { filename: _path2.default.format({ dir: this.baseDir, base: 'catalog.db' }) }, useNullAsDefault: true });this.isReady = false;}initDatabase() {return this.db.schema.createTableIfNotExists('dats', table => {table.string('dat');table.string('name');table.string('dir'); // table.unique('dat');
  ...
  pathToDat(datKey) {return this.db.select('dir').from('dats').where('dat', datKey).first();} // Checks out an item by author and title and dat
  checkout(author, title = false) {if (title) {return this.getDatsWithTitle(author, title).then(rows => this.download(author, title, rows.shift().dat));} // When there is no title specified, get every title
    return this.getTitlesForAuthor(author).map(row => row).each(row => this.download(author, row.title, row.dat));} // Checks out an item by author and title and dat
  download(author, title, datKey) {console.log(`checking out ${author}/${title} from ${datKey}`);return this.dats[datKey].downloadContent(_path2.default.join(author, title));} // Gets a count of authors in the catalog
  ...
  getAuthorLetters() {return this.db.column(this.db.raw('lower(substr(author_sort,1,1)) as letter')).select().from('texts').distinct('letter').orderBy('letter');}getTitlesForAuthor(author) {return this.db('texts').distinct('dat', 'title').where('author', author).orderBy('title');}getDatsWithTitle(author, title) {return this.db('texts').distinct('dat').where('author', author).where('title', title);} // Optionally only include files from a particular dat.
