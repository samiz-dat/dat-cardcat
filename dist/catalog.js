...
class Catalog {constructor(baseDir) {this.pathIsDownloaded = (dat, filePath) => _fs2.default.existsSync(_path2.default.join(dat.directory, filePath));this.baseDir = baseDir;this.dats = [];this.db = new _db2.default(_path2.default.format({ dir: this.baseDir, base: 'catalog.db' })); // If you ever need to see what queries are being run uncomment the following.
  ...
  discoverDats() {return (0, _filesystem.getDirectories)(this.baseDir).map(name => {console.log(`Attempting to load dir: ${_chalk2.default.bold(name)} as a dat`);const opts = { name, createIfMissing: false, // @todo: this was false before, but threw error. find out why?
        sparse: true };return this.importDat(opts);}).then(() => this.cleanupDatsRegistry()).then(() => this.importDatsFromDB()).then(() => this);} // Imports dats listed in the dats table of the database
  ...
  importDat(opts) {if ('key' in opts && opts.key in this.dats) {// The dat is already loaded, we shouldn't reimport it
    ...
    newDat.on('import', (...args) => this.handleDatImportEvent(...args));newDat.on('sync metadata', (...args) => this.handleDatSyncMetadataEvent(...args)); // dw.on('download', (...args) => this.handleDatDownloadEvent(...args));
  ...
  handleDatSyncMetadataEvent(dw) {console.log('metadata synced');_bluebird2.default.map(dw.listContents(), file => this.importDatFile(dw, file));} // When a dat imports a file
