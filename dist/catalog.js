...
class Catalog {constructor(baseDir) {this.baseDir = baseDir;this.dats = [];this.db = new _db2.default(_path2.default.format({ dir: this.baseDir, base: 'catalog.db' }));this.multidat = new _multidat2.default(baseDir); // If you ever need to see what queries are being run uncomment the following.
  ...
  removeDat(key, deleteDir = true) {if (deleteDir) {return this.multidat.pathToDat(key).then(p => {if (p.startsWith(this.baseDir)) {const rimrafAsync = _bluebird2.default.promisify(_rimraf2.default);return this.db.removeDat(key).then(() => this.db.clearTexts(key)).then(() => rimrafAsync(p)).then(() => this.multidat.removeDat(key));}return this.removeDat(key, false);});}return this.multidat.removeDat(key).then(() => this.db.removeDat(key)).then(() => this.db.clearTexts(key));} // ### private functions
