...
class Catalog {constructor(baseDir) {this.getDats = () => this.db('dats').select();this.getDat = key => this.db('dats').select().where('dat', key);this.baseDir = baseDir;this.dats = [];this.db = (0, _knex2.default)({ client: 'sqlite3', connection: { filename: _path2.default.format({ dir: this.baseDir, base: 'catalog.db' }) }, useNullAsDefault: true });this.isReady = false;}initDatabase() {return this.db.schema.createTableIfNotExists('dats', table => {table.string('dat');table.string('name');table.string('dir'); // table.unique('dat');
  ...
  getAuthorLetters() {return this.db.column(this.db.raw('lower(substr(author_sort,1,1)) as letter')).select().from('texts').distinct('letter').orderBy('letter');}getTitlesForAuthor(author) {return this.db('texts').distinct('dat', 'title').where('author', author).orderBy('title');}getDatsWithAuthor(author) {return this.db('texts').distinct('dat').where('author', author);}getDatsWithTitle(author, title) {return this.db('texts').distinct('dat').where('author', author).where('title', title);}getDatsWithFile(author, title, file) {return this.db('texts').distinct('dat').where('author', author).where('title', title).where('file', file);}getDatsWith(opts) {if (opts.author && opts.title && opts.file) {return this.getDatsWithFile(opts.author, opts.title, opts.file);} else if (opts.author && opts.title) {return this.getDatsWithTitle(opts.author, opts.title);} else if (opts.author) {return this.getDatsWithAuthor(opts.author);}return [];}getFilesFromDat(dat) {return this.db('texts').where('dat', dat).orderBy('dat', 'file');} // Optionally only include files from a particular dat.
